...
```{r echo=FALSE, fig.align='center', fig.height=3, fig.width=7, message=FALSE, warning=FALSE, dpi=600}
data_total_semanal %>% 
    filter(
...
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, dpi=600, fig.height=3,fig.width=7}
data_total_semanal %>% 
    filter(
           codigo_de_osinergmin %ni% lista_vecinos,
           fecha >= dmy("01/07/2017") & fecha <= dmy("01/11/2018")) %>% 
...
plot_did <- function(did_obj, aggte_ef) {
  ...
  leadslags_plot %>%
    ...
    annotate(geom='richtext', x=0, y=aggte_ef$overall.att + 0.1, 
             label= paste0("Coeficiente DD = ",round(aggte_ef$overall.att,2),
                           "<b>",signi,"</b>",
                           "<br>(s.e. = ", round(aggte_ef$overall.se,2), ")")
             ) 
}

reg_did <- function(df, producto, bandera) {
  df_producto <- df %>% 
    filter(producto == !!producto) %>% 
    select(codigo_de_osinergmin, fecha, producto, mes, año, precio_de_venta, 
           bandera, tipo_bandera, distrito)
  
  if (bandera == "PROPIA PECSA") {
...
pecsa_g90 <- reg_did(df = data_total_mensual,
                  producto = "G90", 
                  bandera = "PROPIA PECSA" )

plot_did(pecsa_g90$`event study`, pecsa_g90$agregado)
...
pecsa_diesel <- reg_did(df = data_total_mensual,
                  producto = "DIESEL", 
                  bandera = "PROPIA PECSA" )

plot_did(pecsa_diesel$`event study`, pecsa_diesel$agregado)
...
```{r g90-reg-primax}
primax_g90 <- reg_did(df = data_total_mensual,
                  producto = "G90", 
                  bandera = "PROPIA PRIMAX" )

plot_did(primax_g90$`event study`, primax_g90$agregado)
...
```{r diesel_reg_primax}
primax_diesel <- reg_did(df = data_total_mensual,
                  producto = "DIESEL", 
                  bandera = "PROPIA PRIMAX" )

plot_did(primax_diesel$`event study`, primax_diesel$agregado)
```
...
data_g90 <- balancear_panel(data_total_mensual,
                            prod = "G90", 
                            c("01-01-2017", "01-10-2018")) %>%
  filter(producto == "G90") %>%
  mutate(mes = (year(fecha)-2017)*12 + month(fecha),
