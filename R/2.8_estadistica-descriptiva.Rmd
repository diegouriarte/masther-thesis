...
#agregamos columna tiene mecánico
grifos_sc <- grifos_sc %>% 
  mutate(tiene_mecanico = if_else(mecanico + aceite + lavado > 0, 1, 0),
...
precios_tabla <- bind_rows(data_db5, data_g90) %>% 
  group_by(producto) %>%
  summarise(`Media` = mean(precio_de_venta),
            `Des. Est` = sd(precio_de_venta),
            `Min` = min(precio_de_venta),
            `Max` = max(precio_de_venta)) %>%
  mutate("Descripción" = if_else(producto == "DIESEL", 
...
tabla <- grifos_sc %>%
  select(codigo_de_osinergmin, 
         ...
         "Distancia promedio" = distancia_avg,
  ...
  mutate(`Descripción` = fct_relevel(`Descripción`,
...
bind_rows(precios_tabla,tabla) %>%
  select(`Variable`, everything()) %>% 
  kable(format = "html", digits = c(0,0,3,3,1,1)) %>%
  kable_styling(full_width = FALSE, position = "left") %>% 
  pack_rows("Variable dependiente", 1, 2) %>% 
  pack_rows("Características espaciales", 3, 7) %>% 
  pack_rows("Características de la ubicación y del distrito", 8, 13)
...
our_summary1 <-
  list("Agrupamiento espacial" =
       list("min" = ~ min(sc_pre, na.rm = TRUE),
            "max" = ~ max(sc_pre, na.rm = TRUE),
            "mean (sd)" = ~ qwraps2::mean_sd(sc_pre, na_rm = TRUE, 
                                             denote_sd = "paren",
                                             show_n = "never")),
       "Lavado" =
       list("min" = ~ min(lavado, na.rm = TRUE),
            ...
            "mean (sd)" = ~ qwraps2::mean_sd(lavado, 
                                             ...
                                             show_n = "never")),
       "Mecánico" =
       list("min" = ~ min(tiene_mecanico, na.rm =TRUE),
            "max" = ~ max(tiene_mecanico, na.rm = TRUE),
            "mean (sd)" = ~ qwraps2::mean_sd(tiene_mecanico, na_rm = TRUE, 
                                             denote_sd = "paren",
                                             show_n = "never"))
       )
