...
if results.rflag == 1
    tmp = [tmp
           results.prho sqrt(results.rcov)];
end;
...
case {'semi_g'} % <=================== regression model with individual effects
...
fprintf(fid,'\n');
fprintf(fid,'Bayesian regression model with individual spatial effects \n');
if (nflag == 1)
fprintf(fid,'Dependent Variable = %16s \n',vnames(1,:));
end;
fprintf(fid,'R-squared          = %9.4f \n',rsqr);
...
fprintf(fid,'\n');
fprintf(fid,'Bayesian Probit spatial error model \n');
if (nflag == 1)
fprintf(fid,'Dependent Variable = %16s \n',vnames(1,:));
end;
...
tmp = [results.bmean results.bstd];
if results.rflag == 1 % we have a normal prior on rho
tmp = [tmp
       results.prho sqrt(results.rcov)];
end;
...
if results.rflag == 1 % add lambda to the priors
rnames = strvcat(rnames,'lambda');
end;
...
case {'sar_g'} % <=================== Gibbs spatial autoregressive model
...
case {'far_g'} % <=================== Gibbs first-order autoregressive model
...
case {'farj_g'} % <=================== Gibbs first-order autoregressive model
...
fprintf(fid,'\n');
fprintf(fid,'Bayesian First-order spatial autoregressive model \n');
fprintf(fid,'with Jeffreys Priors  \n');
...
fprintf(fid,'acceptance rate    = %9.4f   \n',results.accept2);
fprintf(fid,'min and max rho    = %9.4f,%9.4f \n',results.rmin,results.rmax);
fprintf(fid,'j hyperparameter   = %9.3f   \n',results.j);
